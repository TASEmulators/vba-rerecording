...
static void truncateMovie()
{
	...
	long truncLen = Movie.header.offset_to_controller_data + Movie.bytesPerFrame*(Movie.header.length_frames+1);
...
#		if (defined(WIN32) && !defined(SDL))
			...
			if(checkKey(motion[KEY_DOWN])) buttonData |= BUTTON_MASK_DOWN_MOTION;
			if(checkKey(motion[KEY_UP])) buttonData |= BUTTON_MASK_UP_MOTION;
#		else
#			ifdef SDL
				extern bool sdlCheckJoyKey(int key); // from SDL.cpp
				extern u16 motion[4]; // from SDL.cpp
				if(sdlCheckJoyKey(motion[KEY_LEFT])) buttonData |= BUTTON_MASK_LEFT_MOTION;
				if(sdlCheckJoyKey(motion[KEY_RIGHT])) buttonData |= BUTTON_MASK_RIGHT_MOTION;
				if(sdlCheckJoyKey(motion[KEY_DOWN])) buttonData |= BUTTON_MASK_DOWN_MOTION;
				if(sdlCheckJoyKey(motion[KEY_UP])) buttonData |= BUTTON_MASK_UP_MOTION;
#			endif
#		endif
	...
	{
		// pretend the controller is disconnected (otherwise input it gives could cause desync since we're not writing it to the movie)
		currentButtons[i] = 0;
	}
...
int VBAMovieOpen (const char* filename, bool8 read_only)
{
	...
	if(filename[0] == '\0' || !(file = fopen(filename, read_only ? "rb" : "rb+")))
		{loadingMovie = false; return FILE_NOT_FOUND;}
	else
		fclose(file);
	...
	file = fopen(filename, read_only ? "rb" : "rb+");
...
#	if (defined(WIN32) && !defined(SDL))
		if(theApp.useBiosFile)		Movie.header.optionFlags |= MOVIE_SETTING_USEBIOSFILE;
		if(theApp.skipBiosFile)		Movie.header.optionFlags |= MOVIE_SETTING_SKIPBIOSFILE;
		if(rtcIsEnabled())			Movie.header.optionFlags |= MOVIE_SETTING_RTCENABLE;
...
#	else
		...
		if(useBios)					Movie.header.optionFlags |= MOVIE_SETTING_USEBIOSFILE;
		if(skipBios)				Movie.header.optionFlags |= MOVIE_SETTING_SKIPBIOSFILE;
		if(sdlRtcEnable)			Movie.header.optionFlags |= MOVIE_SETTING_RTCENABLE;
...
int VBAMovieCreate (const char* filename, const char* authorInfo, uint8	startFlags, uint8 controllerFlags, uint8 typeFlags)
{
	...
	if(filename[0]=='\0' || !(file = fopen(filename, "wb")))
		{loadingMovie = false; return FILE_NOT_FOUND;}
	...
	write_frame_controller_data(0); // correct if we can assume the first controller is active, which we can on all GBx/xGB systems
	Movie.currentFrame = 0;
...
void DisplayPressedKeys ()
{
	...
	sprintf(string, "                       ");
		...
		uint32 autoFireKeys = eraseAll ? 0 : (theApp.autoFire|theApp.autoFire2) & (BUTTON_REGULAR_MASK|BUTTON_RECORDINGONLY_MASK);
...
int VBAMovieGetInfo (const char* filename, SMovie* info)
{
	...
	if(filename[0]=='\0' || !(file=fopen(filename, "rb")))
		return FILE_NOT_FOUND;
	...
	else
	{
		...
		fseek(file, 0, SEEK_END);
		int fileSize = ftell(file);
		local_movie.header.length_frames = (fileSize - local_movie.header.offset_to_controller_data) / local_movie.bytesPerFrame - 1;
	}
...
void VBAMovieFreeze (uint8** buf, uint32* size)
{
	...
	*size = 0;
	...
	uint32 size_needed = sizeof(Movie.header.uid) + sizeof(Movie.currentFrame) + sizeof(Movie.header.length_frames);			// room for header.uid, currentFrame, and header.length_frames
