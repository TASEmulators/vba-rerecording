    ...
    if(dataSize == 0) {
      for(j = 0; j < 16; j++) {
		...
		else
		{
			...
			if(num < -9 || num > 99)    r.left -= fontSize.cx;
			if(num >= -99 && num <= 99) buffer.Format("%2d", num);
			else                        buffer.Format("");
		}
        memDC.DrawText(buffer, &r, DT_TOP | DT_LEFT | DT_NOPREFIX);
        r.left = nextRLeft;
    ...
    if(dataSize == 1) {
      for(j = 0; j < 16; j += 2) {
		...
		else
		{
			...
			if(num < -999 || num > 9999)    r.left -= fontSize.cx;
			if(num >= -9999 && num <= 9999) buffer.Format("%4d", num);
			else                            buffer.Format("");
		}
        memDC.DrawText(buffer, &r, DT_TOP | DT_LEFT | DT_NOPREFIX);
        r.left = nextRLeft;
    ...
    if(dataSize == 2) {
      for(j = 0; j < 16; j += 4) {
		...
		else
		{
			...
			if(num >= -9999999 && num <= 99999999) buffer.Format("%8d", num);
			else                                   buffer.Format("");
		}
        memDC.DrawText(buffer, &r, DT_TOP | DT_LEFT | DT_NOPREFIX);
