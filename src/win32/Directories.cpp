...
void Directories::DoDataExchange(CDataExchange* pDX)
{
  ...
  DDX_Control(pDX, IDC_MOVIES_PATH, m_gbromPath);
...
BEGIN_MESSAGE_MAP(Directories, CDialog)
  ...
  ON_BN_CLICKED(IDC_MOVIES_DIR, OnMoviesDir)
  ON_BN_CLICKED(IDC_MOVIES_DIR_RESET, OnMoviesDirReset)
...
BOOL Directories::OnInitDialog() 
{
  ...
  p = regQueryStringValue("moviesdir", NULL);
  if(!p.IsEmpty()) {
    int len = p.GetLength();
    if(len > 0)
      if(p[len-1] == '\\')
        p = p.Left(len-1);
    GetDlgItem(IDC_MOVIES_PATH)->SetWindowText(p);
  }
...
void Directories::OnMoviesDirReset() 
{
  regDeleteValue("moviesdir");
...
void Directories::OnOK() 
{
  ...
  if(!buffer.IsEmpty())
    regSetStringValue("moviesdir", buffer);      
