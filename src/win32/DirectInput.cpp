...
u32 DirectInput::readDevice(int which, bool sensor, bool free)
{
  ...
  if(!free)
	...
	if(sensorOn && !free)
	{
		...
		{
			theApp.sensorX += 3;
			if(theApp.sensorX > 2197) theApp.sensorX = 2197;
			if(theApp.sensorX < 2047) theApp.sensorX = 2057;
		}
		else if(currentButtons[i] & BUTTON_MASK_RIGHT_MOTION)
		{
			theApp.sensorX -= 3;
			if(theApp.sensorX < 1897) theApp.sensorX = 1897;
			if(theApp.sensorX > 2047) theApp.sensorX = 2037;
		}
		else if(theApp.sensorX > 2047)
		{
			theApp.sensorX -= 2;
			if(theApp.sensorX < 2047) theApp.sensorX = 2047;
		}
		else
		{
			theApp.sensorX += 2;
		...
		{
			theApp.sensorY += 3;
			if(theApp.sensorY > 2197) theApp.sensorY = 2197;
			if(theApp.sensorY < 2047) theApp.sensorY = 2057;
		}
		else if(currentButtons[i] & BUTTON_MASK_DOWN_MOTION)
		{
			theApp.sensorY -= 3;
			if(theApp.sensorY < 1897) theApp.sensorY = 1897;
			if(theApp.sensorY > 2047) theApp.sensorY = 2037;
		}
		else if(theApp.sensorY > 2047)
		{
			theApp.sensorY -= 2;
			if(theApp.sensorY < 2047) theApp.sensorY = 2047;
		}
		else
		{
			theApp.sensorY += 2;
