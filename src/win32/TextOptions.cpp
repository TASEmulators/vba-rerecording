...
BOOL TextOptions::OnInitDialog() 
{
	...
	GetDlgItem(IDC_CHECK_TRANSPARENT)->EnableWindow(GetCheckedRadioButton(IDC_RADIO_PREFILTER, IDC_RADIO_POSTRENDER) != IDC_RADIO_POSTRENDER);
...
void TextOptions::OnBnClickedOk()
{
	...
	if(textMethod < 0) textMethod = 0;
	if(textMethod > 2) textMethod = 2;
	if(textColor < 0) textColor = 0;
	if(textColor > 7) textColor = 7;
